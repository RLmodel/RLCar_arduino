...
lane_bin_th = 90     #threshold parameter

warp_src  = np.array([
    [170-warpx_margin, 200-warpy_margin],  
    [30-warpx_margin,  400+warpy_margin],
    [470+warpx_margin, 200-warpy_margin],
    [610+warpx_margin, 400+warpy_margin]
    ], dtype=np.float32)
...
if calibrated:
    mtx = np.array([
    [1427.558258, 0.0, 960.000000], 
    [0.0, 1427.558258, 540.000000], 
    [0.0, 0.0, 1.0]
    ])
...
class Lane(Node):
    ...
    def img_callback(self, data):
        ...
        cv2.circle(image, (170-warpx_margin, 200-warpy_margin), 10, (0,255,0), -1)
        cv2.circle(image, (30-warpx_margin,  400+warpy_margin), 10, (0,255,0), -1)
        cv2.circle(image, (470+warpx_margin, 200-warpy_margin), 10, (0,255,0), -1)
        cv2.circle(image, (610+warpx_margin, 400+warpy_margin), 10, (0,255,0), -1)
        ...
        text1 = str(f'Sx : {self.Sx}')
        cv2.putText(lane_img, text1, (240, 50), cv2.FONT_HERSHEY_SIMPLEX,  0.8, (0,0,0),thickness=2)
        text2 = str(f'err : {self.err}')
        cv2.putText(lane_img, text2, (240, 80), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0,0,0), thickness=2)
        if self.Sx > 4:
            cv2.putText(lane_img, 'right', (270, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0,0,0), thickness=2)
        elif self.Sx <- 4:
            cv2.putText(lane_img, 'left', (270, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0,0,0), thickness=2)    
        else:
            cv2.putText(lane_img, 'straight', (270, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0,0,0), thickness=2) 
...
writer = cv2.VideoWriter('hello2.avi', fourcc, 24, (int(Width), int(Height)))
