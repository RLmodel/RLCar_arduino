...
class ImageSubscriber(Node):
    ...
    def steering_callback(self, msg):
        if self.state == StateMachine.LANE:   
            # len_front = msg.ranges[0]               # degree(0 ~ 359) == ranges[0 ~ 719] CCW
            # len_left15 = msg.ranges[30]              # 15 degree (left)  
            # len_right15 = msg.ranges[690]           # 15 degree (right)

            error = self.error*self.error_weight                      
            twist_msg = Twist()
            # self.get_logger().info("distance : {}".format(self.len_front))
...
def draw_lines(frame, lines):
    ...
    cv2.putText(frame, f":steer: {leftright}", (200, 20), cv2.FONT_HERSHEY_SIMPLEX, 1, (100, 200, 100), 2)
